    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        if (user == null) {
            ...
            if (StringUtils.equals(passwordEncode, user.getUsrPassword())) {
                ...
                HttpSession existingSession = HttpSessionCollector.find(username);
                if (existingSession != null && !existingSession.getId().equals(getCurrentSessionId())) {
                    // Existing session found, invalidate the previous session
                    existingSession.invalidate();
                }

                if (HttpSessionCollector.find(username) != null) {
                    ...
                    throw new DisabledException(
                            "Your account is being used/locked. Please re-login again after " + valueTimeout
                                    + " minutes or contact your Web Application Administrator to unlock your account.");
                } else {
                ...
                if (user.getLoginAttempts() != null && user.getLoginAttempts() >= MAX_LOGIN_ATTEMPTS) {
                    throw new BadCredentialsException("Your account has been blocked due to " + MAX_LOGIN_ATTEMPTS
                            + " or more wrong input of password, please request password reset in order to login your account.");
