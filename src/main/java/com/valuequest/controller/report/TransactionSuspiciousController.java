...
@Controller
@RequestMapping("/report/suspicious")
public class TransactionSuspiciousController extends BaseController {

	final static String MENU 		= "REPORT";
	final static String PRIVILEDGE 	= "TRANSACTION_SUSPICIOUS";
	final static String RPT_MENU	= "suspicious";
	final static String RPT_TITLE 	= "Suspicious Transaction Report";
	private String BASE_VIEW 		= "08.report/";
	private String LIST_VIEW 		= "report-list";
	private String GENERATE_VIEW	= "param-suspicious";
 ...
 SecUser user = this.getLoginSecUser(session);
	...
	public @ResponseBody DataTables search(DataTables dataTables, 
		...
		searchMap.put("type", AsynReport.REPORT_TYPE_TRANS_SUSPICIOUS);
	...
	public String generate(Model model, HttpSession session) {
		...
		if (getPriviledgeUser(session, PRIVILEDGE, NEW)) {
			...
			model.addAttribute("listSuspiciousType", genericService.lookup(Lookup.LOOKUP_SUSPICIOUS_TYPE));
	...
	public @ResponseBody AjaxResponse save(@RequestBody TransactionParam param, HttpSession session) {
		...
		report.setType(AsynReport.REPORT_TYPE_TRANS_SUSPICIOUS);
