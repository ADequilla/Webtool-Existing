...
@Controller
@RequestMapping("/report/pinpsswd-change")
public class MpinPasswdChangeController extends BaseController {

	final static String MENU 		= "REPORT";
	final static String PRIVILEDGE 	= "PINPASSWORD_CHANGE";
	final static String RPT_MENU	= "pinpsswd-change";
	final static String RPT_TITLE 	= "MPIN - Password Change Report";
	private String BASE_VIEW 		= "08.report/";
	private String LIST_VIEW 		= "report-list";
	private String GENERATE_VIEW	= "param-pinpsswd-change";
 ...
 SecUser user = this.getLoginSecUser(session);
	...
	public @ResponseBody DataTables search(DataTables dataTables, 
		...
		searchMap.put("type", AsynReport.REPORT_TYPE_PINPASSWD_CHANGE);
	...
	public String generate(Model model, HttpSession session) {
		...
		if (getPriviledgeUser(session, PRIVILEDGE, NEW)) {
			...
			model.addAttribute("listModifType", genericService.lookup(Lookup.LOOKUP_MODIFICATION_TYPE));
//			model.addAttribute("listTransStatus", genericService.lookup(Lookup.LOOKUP_TRANSACTION_STATUS));
	...
	public @ResponseBody AjaxResponse save(@RequestBody TransactionParam param, HttpSession session) {
		...
		String params = param.getTransDateStart() + "|" + 
						...
						param.getModifType() +"|END";

		AsynReport report = new AsynReport();
		report.setType(AsynReport.REPORT_TYPE_PINPASSWD_CHANGE);
