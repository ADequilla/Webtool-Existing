...
public class ActivityHistoryController extends BaseController {
	...
	final static String PRIVILEDGE 	= "ACTIVITY_HISTORY";
	final static String RPT_MENU	= "activity-history";
	final static String RPT_TITLE 	= "Activity History Report";
	private String BASE_VIEW 		= "08.report/";
	private String LIST_VIEW 		= "report-list";
	private String GENERATE_VIEW	= "param-activity-history";
	...
	public @ResponseBody DataTables search(DataTables dataTables, 
		...
		searchMap.put("type", AsynReport.REPORT_TYPE_ACTIVITY_HISTORY);
	...
	public String generate(Model model, HttpSession session) {
		...
		if (getPriviledgeUser(session, PRIVILEDGE, NEW)) {
			List<Module> listModul = genericService.module();
			for(int i = 0 ; i < listModul.size() ; i++){
				if(listModul.get(i).getName().equals("CURRENCY_INFO") 
						|| listModul.get(i).getName().equals("BALANCE_INQUERY")
						|| listModul.get(i).getName().equals("TYPE_CHANGE")){
					listModul.remove(i);
					i--;
				}else if(listModul.get(i).getName().equals("NEAREST_ATM_LOCATION")){
					listModul.get(i).setName("ATM_LOCATION");
				}
	...
	public @ResponseBody AjaxResponse save(@RequestBody TransactionParam param, HttpSession session) {
		...
		String params = param.getTransDateStart() + "|" + 
						...
						param.getBranchCode() + "|" + 
						param.getActivity() + "|" + 
						param.getClientType() + "|END";

		AsynReport report = new AsynReport();
		report.setType(AsynReport.REPORT_TYPE_ACTIVITY_HISTORY);
