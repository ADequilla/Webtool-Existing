...
<script type="text/javascript">
        $(document).ready(function() {
			...
			$("#btn-reset").click(function(){
				$('#providerName]').val($('#name_default').val());
            	$('#providerDescription').val($('#description_default').val());
            	$('#providerStatus').val($('#status_default').val());

            $("#btn-save").click(function(){
                if ($("form").valid()) {
                	var dataJson = $('form').serializeObject();
                    submit('/monitoring/bill-payment-provider/save', JSON.stringify(dataJson), function (data) {
	...
	<div class="content">
			...
			<div class="col-lg-6 ">
				<div class="main-header">
					<ul class="breadcrumb">
						...
						<li>Bill Payment</li>
						<li><a
							href="${pageContext.request.contextPath}/utilities/fee/">Provider</a></li>
		...
		<form class="form-horizontal" role="form" method="post">
			<input type="hidden" name="id" value="${provider.id}"> <input
				type="hidden" id="name_default" value="${provider.name}"> <input
				type="hidden" id="description_default"
				value="${provider.description}"> <input type="hidden"
				id="status_default" value="${provider.status}">
