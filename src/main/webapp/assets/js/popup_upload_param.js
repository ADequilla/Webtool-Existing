...
(function( $ ) {
    function PopupUploadParam() {
        ...
        this.createModal = function createModal() {
        	...
        	html += '<div class="modal-header text-left">';
        	 opts.modal_title ='Upload New File';
            if (opts.category == 2){
           	 opts.modal_title ='Upload Edit File';
            }
        	...
        	html += '<input type="hidden" id="category" name="category" value="'+opts.category+'"/>';        	 
        	html += '</div>';
    ...
    $.fn.popup_upload_param = function( options, callback ) {
        var opts = $.extend( {}, $.fn.popup_upload_param.defaults, options );
        var jqThis = $(this);

        var object = new PopupUploadParam();
...
$.fn.popup_upload_param.defaults = {
    container: undefined,
